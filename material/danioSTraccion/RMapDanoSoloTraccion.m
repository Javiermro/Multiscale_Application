function [ct,sigma_new,eps_new,hvar_new,aux_var] = RMapDanoSoloTraccion(eps_new,hvar_old,aux_var,...
   e_DatMatSet,e_VG)
...
if e_VG.ndime==2
   ...
   m_StressEfecPos = f_AutoValPos2D(effective_stress,e_VG);
else
   error('Daño solo tracción: No definido los autovalores positivo en el caso 3D.')
end
% Estado Trial
r_trial = sqrt(eps_new'*m_StressEfecPos);
...
%       end  
   ...
   if q_new<zero_q
      ...
      elseif (tita == 1)
         %Parece estar mal beta 3, ya que es Hactual = H*exp(H*(r_new-r_0)/q_0) y no H0(=H)!!!!!!!!!
         %beta_3  = -q_new/r_new^3*(-H*r_new/q_0 + 1);
         beta_3  = -q_new/r_new^3*(-H*exp(H*(r_new-r_0)/q_0)*r_new/q_0 + 1);
      end
   end
