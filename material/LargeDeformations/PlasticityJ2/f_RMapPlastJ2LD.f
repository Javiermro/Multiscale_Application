...
C     Variables para calcular la energia
      ...
      real*8 DTRES,YOUNG,POISS,MULAM,LAMBL2,KAPPA,trLogVP,VPdev(vectorDim)
...
c     Elastic strain energy
     ...
     .      (DLOG(VP(3)))**2 ) + LAMBL2*(DLOG(VP(1)*VP(2)*VP(3)))**2 
      ...
      VPdev(1) = 2.D0/3.D0*DLOG(VP(1)) - 1.D0/3.D0*(DLOG(VP(2))+DLOG(VP(3)))
      VPdev(2) = 2.D0/3.D0*DLOG(VP(2)) - 1.D0/3.D0*(DLOG(VP(1))+DLOG(VP(3)))
      VPdev(3) = 2.D0/3.D0*DLOG(VP(3)) - 1.D0/3.D0*(DLOG(VP(2))+DLOG(VP(1)))
