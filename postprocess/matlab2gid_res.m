...
%          STRESS= reshape(STRESS,ntens*NPG_ELE,[]);
   ...
   if eltype~=[21,22,23]
      ...
      if e_VG.esME
         ...
         f_ImprTensPG(fid_res,iStep,struhyp,ntens,1,nomGaussUnicoSet,s_tipoDato,m_NumElem,DefMacro{iSet})
      end
      %
      %             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      %Impresión del jacobiano para el problema de grandes deformaciones
      %Se calcula el jacobiano del todo el gradiente de deformación y para el gradiente deformación total
      %micro para el problema multiescala
      if struhyp==20
         strainPG = reshape(strain,ntens,[],nElem);
         m_J = zeros(NPG_ELE,nElem);
         for iElem = 1:nElem
            %Se descarta los PGs arriba de los estándars.
            for iPG = 1:NPG_ELE
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   switch conshyp 
      ...
      case {50,55}
        ...
        sih=e_DatSet.e_DatMat.e_DatSet(1).e_DatMat.sihvarpg; % solo anda si el sihvarpg es igual en todos los sets de la micro
        ig = 0; hvar_new2 = [] ; medH =[]; %medA=[];
        for iElem = 1:nElem
         mm=[];
         for iPG = 1:NPG_ELE
            ig = ig + 1 ; sumS = 0; %sumA=0;             
            for is=1:e_DatMatSet.e_VG.nSet
                sumS = sumS + sum(hvar_new(ig).e_VarEst(is).hvar,2) ;
...
%             medA = sumA/size(e_DatMatSet.m_SetElem,2) ;
        ...
        sihvarpg=sih ;
         ...
         fprintf(fid_res,format,[m_NumElem;hvar_new2(1:sihvarpg:end,:)]);
         fprintf(fid_res,'End Values\n');
